# Homework NoSQL 
Для успешного выполнения ДЗ вам потребуется использовать базы данных Redis, Cassandra/Scylla, ClickHouse и MongoDB.
...
### MongoDB
Команда для старта MongoDB:
```bash
docker run -d --name mongodb -p 27017:27017 mongo
```
Зайти в консольный клиент MongoDB:
```bash
...
### 4. MongoDB (Document Store) - опциональное задание, никак не влияет на оценку за ДЗ

**Описание**: MongoDB - это документо-ориентированная NoSQL база данных, хранящая данные в формате BSON (похож на JSON).

**Документация**: https://www.mongodb.com/docs/

**Задание**: В папке `mongodb` находится Maven проект. В зависимостях подключен MongoDB Java Driver.

Структура документа в коллекции `posts`:
```json
{
...
**Задачи**: В классе `BlogService` необходимо реализовать следующие методы:

1. `findPostsWithMoreThanNComments(int minComments)` - найти посты, у которых больше N комментариев

2. **`getTotalViews()`** - **получить суммарное количество просмотров всех постов**  
   ВАЖНО: Не все посты имеют поле `views`! Решение должно корректно обрабатывать отсутствующие поля. 

3. **`addRatingToCommentsWithoutIt(String postTitle)`** - **добавить поле `rating=0` к комментариям, у которых его нет**
